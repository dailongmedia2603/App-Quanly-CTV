...
const CreateEmailContentTab = () => {
  ...
  if (view === 'history') {
    return (
      <Card>
        <CardHeader><div className="flex justify-between items-center"><CardTitle>Nội dung mail đã tạo</CardTitle><Button onClick={() => setView('create')} variant="outline"><ArrowLeft className="mr-2 h-4 w-4" />Quay lại</Button></div></CardHeader>
        <CardContent>{loadingHistory ? <p>Đang tải...</p> : history.map(item => <div key={item.id} className="mb-4 p-4 border rounded-lg"><h3>{item.name}</h3><p><strong>Tiêu đề:</strong> {item.subject}</p></div>)}</CardContent>
      </Card>
    );
  }
