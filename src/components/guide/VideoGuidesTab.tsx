...
const VideoGuidesTab = () => {
  ...
  const handleEditClick = (guide: VideoGuide) => { setEditingGuide(guide); setTitle(guide.title); setDescription(guide.description || ''); setYoutubeUrl(guide.youtube_url); setThumbnailFile(null); setIsDialogOpen(true); };
  ...
  const handleSave = async () => {
    ...
    if (thumbnailFile) {
      const filePath = `public/${Date.now()}-${thumbnailFile.name}`;
