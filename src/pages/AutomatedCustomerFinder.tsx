...
const AutomatedCustomerFinder = () => {
  ...
  const fetchReportData = async () => {
    ...
    const { data: reports, error: reportsError } = await supabase.functions.invoke(
      'get-facebook-posts-for-internal-finder'
    );
  ...
  return (
    <>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold">Tìm KH tự động (Nội bộ)</h1>
          <p className="text-gray-500 mt-1">Dữ liệu khách hàng tiềm năng từ các chiến dịch quét nội bộ.</p>
        </div>
        <Card className="border-orange-200">
          <CardHeader>
            ...
            <CardDescription>Danh sách các khách hàng tiềm năng được thu thập từ các chiến dịch nội bộ.</CardDescription>
          </CardHeader>
          <CardContent>
            {isMobile ? (
