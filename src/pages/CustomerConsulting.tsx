...
import { Plus, MessageSquare, Send, Bot, User as UserIcon, Trash2, Pencil, Check, History, Copy, RefreshCw, ArrowLeft } from 'lucide-react';
...
const CustomerConsulting = () => {
  ...
  const renderMobileLayout = () => (
    <div className="h-[calc(100vh-8rem)] flex flex-col">
      {!activeSession ? (
        // Session List View
        <div className="flex flex-col h-full p-4 bg-white rounded-lg border border-orange-200">
          ...
          <ScrollArea className="flex-grow -mr-4">
            <div className="space-y-2 pr-4">
              {sessions.map(session => (
                <div key={session.id} onClick={() => handleSelectSession(session)} className="p-3 rounded-lg cursor-pointer border hover:bg-gray-50">
                  <div className="flex items-start justify-between">
                    ...
                    <Button variant="ghost" size="icon" className="h-6 w-6 text-red-500 flex-shrink-0" onClick={(e) => { e.stopPropagation(); handleDeleteClick(session); }}><Trash2 className="h-3 w-3" /></Button>
                  </div>
                </div>
              ))}
            </div>
        ...
        // Chat View
        <div className="flex flex-col h-full bg-white rounded-lg border border-orange-200">
          <div className="p-2 border-b flex-shrink-0 flex items-center justify-between">
            <Button variant="ghost" size="icon" className="mr-2" onClick={() => setActiveSession(null)}>
              <ArrowLeft className="h-5 w-5" />
            </Button>
            <div className="flex-1 min-w-0">
              <h3 className="font-bold truncate">{activeSession.title}</h3>
              <div className="flex items-center gap-1 mt-1 flex-wrap">
                {activeSession.service_names?.map(name => <Badge key={name} variant="secondary" className="text-xs">{name}</Badge>)}
              </div>
            </div>
            <Select value={customerSalutation} onValueChange={(value: 'Anh' | 'Chị' | 'A/C') => { if (value) handleSalutationChange(value); }}>
              <SelectTrigger className="w-[100px] h-8 text-xs ml-2"><SelectValue placeholder="Xưng hô" /></SelectTrigger>
              <SelectContent><SelectItem value="Anh">Anh</SelectItem><SelectItem value="Chị">Chị</SelectItem><SelectItem value="A/C">A/C</SelectItem></SelectContent>
            </Select>
          </div>
            ...
            <div className="space-y-6">
              {messages.map((msg, index) => (
                <div key={index} className={cn("flex flex-col gap-1", msg.role === 'user' ? 'items-end' : 'items-start')}>
                  <p className="text-xs text-gray-500 px-1">{msg.role === 'user' ? 'Khách hàng' : 'AI trả lời'}</p>
                  <div className={cn("flex items-center gap-2 w-full group", msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                    ...
                    {msg.role === 'assistant' && (
                        <div className="flex flex-col self-center space-y-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button variant="ghost" size="icon" className="h-7 w-7 bg-gray-100 hover:bg-gray-200" onClick={() => handleCopy(msg.content)}><Copy className="h-4 w-4 text-gray-600" /></Button>
                            <Button variant="ghost" size="icon" className="h-7 w-7 bg-orange-100 hover:bg-orange-200" onClick={() => handleRegenerateClick(msg)}><RefreshCw className="h-4 w-4 text-brand-orange" /></Button>
                        </div>
                    )}
                  ...
                  <div key={session.id} onClick={() => handleSelectSession(session)} className={cn("p-3 rounded-lg cursor-pointer border", activeSession?.id === session.id ? "bg-brand-orange-light border-brand-orange" : "hover:bg-gray-50")}>
                    {editingTitle?.id === session.id ? (
                      <div className="flex items-center space-x-2">
                        <Input value={editingTitle.title} onChange={e => setEditingTitle({ ...editingTitle, title: e.target.value })} onKeyDown={handleTitleKeyDown} autoFocus className="h-8" />
              ...
              <>
                <div className="p-4 border-b flex-shrink-0 flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Select value={customerSalutation} onValueChange={(value: 'Anh' | 'Chị' | 'A/C') => { if (value) handleSalutationChange(value); }}>
                      <SelectTrigger className="w-[140px]"><SelectValue placeholder="Chọn xưng hô" /></SelectTrigger>
                      <SelectContent><SelectItem value="Anh">Anh</SelectItem><SelectItem value="Chị">Chị</SelectItem><SelectItem value="A/C">A/C (Chưa rõ)</SelectItem></SelectContent>
                    </Select>
                  </div>
                </div>
                <ScrollArea className="flex-grow p-4 bg-gray-50/50">
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
        <AlertDialogContent>
            ...
            <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={confirmDeleteSession} className="bg-red-600 hover:bg-red-600">Xóa</AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
