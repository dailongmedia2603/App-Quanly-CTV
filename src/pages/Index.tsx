...
interface FacebookGroupCategory {
  id: string;
  name: string;
  list_nguon_facebook: {
...
const Index = () => {
  ...
  const [facebookGroupCategories, setFacebookGroupCategories] = useState<FacebookGroupCategory[]>([]);
  ...
  const facebookGroupsOptions = useMemo(() => {
    return facebookGroupCategories.map(category => ({
      label: category.name,
      options: category.list_nguon_facebook
  ...
  const fetchFacebookGroups = async () => {
    ...
    const { data, error } = await supabase
      .from('facebook_group_categories')
      .select('id, name, list_nguon_facebook(group_id, group_name)')
      .order('name');
    
    if (error) {
      showError("Không thể tải danh sách group.");
    } else {
      setFacebookGroupCategories(data as FacebookGroupCategory[]);
    }
