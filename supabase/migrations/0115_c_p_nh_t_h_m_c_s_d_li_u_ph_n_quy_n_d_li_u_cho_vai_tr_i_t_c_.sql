CREATE OR REPLACE FUNCTION public.get_all_facebook_posts_for_finder()
 RETURNS TABLE(id uuid, campaign_id uuid, posted_at timestamp with time zone, keywords_found text[], ai_evaluation text, sentiment text, source_url text, scanned_at timestamp with time zone, description text, suggested_comment text, identified_service_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
...
BEGIN
  ...
  IF 'Super Admin' = ANY(user_roles) OR 'Quản lý' = ANY(user_roles) THEN
    RETURN QUERY
    SELECT
      f.id, f.campaign_id, f.posted_at, f.keywords_found, f.ai_evaluation, f.sentiment, f.source_url, f.scanned_at, f.content AS description, f.suggested_comment, f.identified_service_id
